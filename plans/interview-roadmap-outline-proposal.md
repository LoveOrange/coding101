...
这套路线不追求堆出一份无限长的“八股题库”，而是让绝大多数真实面试问题都能稳定映射到一个 Topic，并能从概念、原理、实践、排障和方案取舍逐层深入。

- **覆盖目标**：覆盖目标岗位约 90% 的常见技术面试考点。“90%”是内容设计目标，不冒充严格统计结论；验收口径是抽样面经中的问题至少 90% 能归入现有 Topic，剩余问题多为公司业务、特定工具或研究前沿。
- **能力等级**：Junior 侧重正确使用和解释基本机制；Middle 侧重原理、工程落地和排障；Senior 侧重架构权衡、生产治理、跨团队决策和复杂故障复盘。
- **面试优先级**：高频、重点、扩展三级继续沿用。等级与优先级是两个维度，例如“数据库索引”可同时是 Junior/Middle 且高频，“编译器优化”可为 Senior 且扩展。
- **组织方式**：共享基础只维护一份，岗位路线通过 Topic 引用进行组合；岗位页只保留该岗位独有的语言、框架、平台和场景。
- **内容单元**：每个 Topic 后续至少包含学习目标、必会问题、原理链路、实战/排障题、Senior 追问、相关文章与练习。
...
8. **Senior 04｜架构领导力**：演进路线；兼容与迁移；多租户；安全与合规；可运维性；架构决策记录；失败方案复盘；组织结构对架构的影响。
...
4. **Middle 02｜技术沟通**：向不同受众解释复杂问题；设计评审；代码评审分歧；推动依赖团队；书面决策与风险同步。
5. **Senior 01｜领导力与影响力**：项目拆解；优先级；授权与辅导；跨团队协作；招聘/面试；质量文化；技术债和长期演进。
6. **Senior 02｜业务与技术决策**：从用户/收入/成本/风险解释技术价值；build vs buy；速度与质量；短期交付与长期架构平衡。
7. **通用｜面试过程**：现场编码沟通；系统设计表达；反问；薪酬/动机/离职原因；面后记录与知识缺口回填。
...
8. **Senior｜架构与演进**：DDD 边界、模块化单体与微服务、容量与高可用、数据拆分、异步化、容灾、多租户、性能/成本；经典系统设计与技术领导力。

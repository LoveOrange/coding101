...
**相关内容**：[排序规则决定键的位置](#1-排序规则决定键的位置)至[compare 返回 0 就表示同一个键](#5-compare-返回-0-就表示同一个键)。
...
**相关内容**：[compare 返回 0 就表示同一个键](#5-compare-返回-0-就表示同一个键)、[TreeSet 复用有序 Map 语义](#6-treeset-复用有序-map-语义)。

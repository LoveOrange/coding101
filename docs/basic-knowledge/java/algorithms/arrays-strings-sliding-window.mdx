...
**相关内容**：[确定半开区间和不变量](#2-确定半开区间和不变量)、[可变窗口处理最长无重复子串](#4-可变窗口处理最长无重复子串)、[Java String 的索引单位](#5-java-string-的索引单位)。
...
**相关内容**：[先判断问题是否要求连续区间](#1-先判断问题是否要求连续区间)、[怎样证明窗口没有漏解](#7-怎样证明窗口没有漏解)。

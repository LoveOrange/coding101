...
**相关内容**：[从 Spliterator 开始拆分数据](#1-从-spliterator-开始拆分数据)、[ForkJoinPool 执行分治任务](#2-forkjoinpool-执行分治任务)。
...
**相关内容**：[外部可变状态会产生数据竞争](#4-外部可变状态会产生数据竞争)、[阻塞 I/O 会占住共享 Worker](#5-阻塞-io-会占住共享-worker)、[什么情况下可能获得收益](#7-什么情况下可能获得收益)。

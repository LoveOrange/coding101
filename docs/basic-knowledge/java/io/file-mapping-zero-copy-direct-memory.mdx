...
**相关内容**：[FileChannel.transferTo 减少用户态复制](#2-filechanneltransferto-减少用户态复制)、[内存映射把文件区域映射到地址空间](#3-内存映射把文件区域映射到地址空间)、[三种机制优化的环节](#5-三种机制优化的环节)。
...
**相关内容**：[直接 ByteBuffer 使用堆外内存](#4-直接-bytebuffer-使用堆外内存)。

...
**相关内容**：[Buffer 保存读写状态](#1-buffer-保存读写状态)、[Channel 在端点与 Buffer 之间搬运数据](#2-channel-在端点与-buffer-之间搬运数据)、[Selector 等待多个通道就绪](#3-selector-等待多个通道就绪)。
...
**相关内容**：[就绪不等于操作已经完成](#31-就绪不等于操作已经完成)、[BIO、NIO 与异步 I/O](#6-bionio-与异步-io)。

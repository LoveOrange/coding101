...
**相关内容**：[Serializable 怎样工作](#1-serializable-怎样工作)、[serialVersionUID 表示序列化版本身份](#2-serialversionuid-表示序列化版本身份)。
...
**相关内容**：[反序列化不可信数据的风险](#4-反序列化不可信数据的风险)、[使用反序列化过滤器限制对象图](#5-使用反序列化过滤器限制对象图)。

...
**相关内容**：[`ServiceLoader` 按服务接口发现提供者](#1-serviceloader-按服务接口发现提供者)和[线程上下文类加载器提供向下查找入口](#2-线程上下文类加载器提供向下查找入口)。
...
**相关内容**：[插件隔离需要明确共享包和私有包](#3-插件隔离需要明确共享包和私有包)至[卸载的单位是类加载器及其所有类](#6-卸载的单位是类加载器及其所有类)。
